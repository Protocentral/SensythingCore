...
################################################################################
#
#  SensythingCore - Compile & Upload Cap WiFi App
#
#  Compiles and uploads the Cap_WiFi_App example to a connected
#  Sensything Cap (ESP32-S3) board. Uses the huge_app partition scheme
#  required by the full embedded web dashboard.
#
#  Prerequisites:
#    - arduino-cli installed and in PATH
#    - ESP32 board package installed
#    - Required libraries: WebSockets, ProtoCentral FDC1004
#    - Sensything Cap connected via USB
#
#  Usage:
#    ./upload-cap-wifi-app.sh [--port PORT] [--compile-only] [--verbose]
#
#  Options:
...
SKETCH_DIR="$PROJECT_ROOT/examples/03.Applications/Cap_WiFi_App"
PORT=""
COMPILE_ONLY=false
VERBOSE=false
...
if [ ! -f "$SKETCH_DIR/Cap_WiFi_App.ino" ]; then
    echo -e "${RED}Error: Sketch not found at $SKETCH_DIR/Cap_WiFi_App.ino${NC}"
    exit 2
fi

echo ""
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Sensything Cap - WiFi App Compile & Upload${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo ""

# Compile
print_step "Compiling Cap_WiFi_App..."
