...
//
// Sensything OX - Complete WiFi Application
//
...
const unsigned long DEFAULT_SAMPLE_RATE_MS = 8;    // 8ms ≈ 125Hz (adjustable via dashboard)
...
void setup() {
    // -------------------------------------------------------------------------
    // 1. Initialize Platform (Sensor + Serial)
    // -------------------------------------------------------------------------
    Serial.println();
    Serial.println("=================================================================");
    Serial.println("   SensythingOX - Complete WiFi Application");
    Serial.println("=================================================================");
    Serial.println();
    ...
    if (sensything.initAPStation(AP_SSID, AP_PASSWORD, PRECONFIGURED_SSID, PRECONFIGURED_PASSWORD)) {
        ...
        Serial.println("=================================================================");
        Serial.println();
        Serial.println("   • Real-time PPG waveform chart (IR/RED)");
        Serial.println("   • Live SpO2 and Heart Rate displays");
        Serial.println("   • Signal quality indicators");
        Serial.println("   • WiFi network scanner & configurator");
        Serial.println("   • Sample rate control (50-125 Hz)");
